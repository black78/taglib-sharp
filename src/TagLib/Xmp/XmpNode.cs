...
#region Public Methods
		...
		public void RenderInto (XElement parent)
		{
			if (IsRootNode) {
				...
				XmpTag.AddAttribute(parent, Name, Namespace, Value);

			} else if (Type == XmpNodeType.Simple || Type == XmpNodeType.Struct) {
				var node = XmpTag.AddNode (parent, Name, Namespace);
				node.Value = Value;
				...
				if (Type == XmpNodeType.Struct) {
					...
					XmpTag.AddAttribute(node, XmpTag.PARSE_TYPE_URI, XmpTag.RDF_NS, "Resource");
				}
				...
				var bag = XmpTag.AddNode(node, XmpTag.BAG_URI, XmpTag.RDF_NS);
				...
				if (QualifierCount > 0)
					throw new NotImplementedException ();
				var bag = XmpTag.AddNode (node, XmpTag.ALT_URI, XmpTag.RDF_NS);
				foreach (var child in Children)
					child.RenderInto (bag);
				...
				if (QualifierCount > 0)
					throw new NotImplementedException ();
				var bag = XmpTag.AddNode (node, XmpTag.SEQ_URI, XmpTag.RDF_NS);
				foreach (var child in Children)
					child.RenderInto (bag);
		...
		private void AddAllQualifiersTo (XElement xml)
		{
			...
			foreach (var collection in qualifiers.Values) {
				foreach (XmpNode node in collection.Values) {
					XmpTag.AddAttribute(xml, node.Name, node.Namespace, node.Value);
				}
			}
		}
